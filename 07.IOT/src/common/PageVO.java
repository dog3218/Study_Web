...
public class PageVO {
	...
	public void setTotalList(int totalList) {
		this.totalList = totalList; //DB로부터 조회된 총 글 건수
		//총 페이지수
		totalPage = totalList / pageList; //총 페이지수 = 총 건수 / 페이지당 글 건수
		if (totalList % pageList > 0) ++ totalPage;
		
		totalBlock = totalPage / blockPage; //총 블럭수 = 총 페이지수 / 블럭당 페이지수
		if(totalPage % blockPage > 0) ++ totalBlock;
		
		//현재 페이지에 따라 보여질 글의 시작/끝 목록번호가 결정됨.
		//각 페이지의 끝 목록번호 : 총 목록수 - (페이지번호-1) * 페이지당 보여질 목록수
		endList = totalList - (curPage -1) * pageList; 
		//각 페이지의 시작 목록번호 : 끝 목록번호 - (페이지당 보여질 목록수 -1)
		beginList = endList - (pageList -1); 
		
		/*1페이지에 1부터 나오게 하려면 이렇게 하면됨
		 * endList = curPage * pageList; //10     ------50  1   10 
		//각 페이지의 시작 목록번호 : 끝 목록번호 - (페이지당 보여질 목록수 -1)
		beginList = (curPage-1)*pageList+1; //1
*/		
		//각 블럭에 보여질 페이지 번호는 현재 블럭에 따라 결정됨
